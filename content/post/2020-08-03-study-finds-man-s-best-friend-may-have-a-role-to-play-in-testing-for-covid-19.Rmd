...
tags:
  ...
  - statistics
subtitle: ''
description: "Bayesian analysis of the sensitivity, specificity, TPR, and TNR distributions from a German study measuring dogs' ability to detect SARS-CoV-2 from saliva and tracheobronchial secretions"
image: ''
---
...
Scatterplots below show the dogs' measures of sensitivity, specificity, true positive rate (TPR) and true negative rate (TNR), each calculated from the true positive (TP), true negative (TN), false positive (FP), and false negative (FN) observations in Table 1. 

```{r plot_dat2, echo = FALSE}
plot_dat = dat %>% select(dog_name, sens, spec, tpr, tnr) %>% 
  pivot_longer(cols = c(sens, spec, tpr, tnr)) %>% 
  group_by(name) %>% 
