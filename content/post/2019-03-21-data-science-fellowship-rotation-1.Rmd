...
The tool that I was working with, *ihazr* (interactive hazard regression), was built by Jason Liang using [R Shiny](https://shiny.rstudio.com/) and the [htmlwidgets](https://www.htmlwidgets.org/) package. *ihazr*, as its name suggests, is an interactive tool used for estimating the hazard function in survival analysis. While the statistical theory involved in survival analysis is extensive, there were only a few concepts I needed to fully understand for this project.
...
Simply put, [D3.js](https://d3js.org/) is amazing. If Picasso were a data scientist, I'm pretty sure he would use it. The D3 framework allows users to bind their data to the html document, so it allows for easy transfer between actual data and web visualization. D3 allows for customizable, clickable, actionable data that you can see and investigate, so it provides the perfect interface for the **ihazr** survival diagnostic tool.  
...
* d3.js data entering
    ...
    *data( )* and *enter( )* are the PB & J of D3.js. In order to bind data to the DOM (document object model) D3 using these functions to attach data to a particular web element and then to display that data in a specified form (text, circles, rectangles, etc.) Here's an example. In the *ihazr* package, these functions are typically used for any display of the uploaded survival data and supporting visualizations (e.g. bandwidth selector). 
...
Currently, *ihazr* is available on CRAN. Run the example code below to check it out! 

```{r, eval=F}
install.packages('ihazr')
library(ihazr)
library(survival)
library(dplyr)
...
Though it was frustrating at times, it's been an amazing journey to supplement new code and visualizations to *ihazr*. I've also enjoyed leading the package submission of *ihazr* and finally seeing it available on CRAN. I'm sure like many other R users, I typically take the packages that are built for granted and rarely contemplate the time and effort teams have taken to build these packages. Through this experience, I've been able to practice some of my statistical background and build some muscles in coding and software development along the way. I've gained a new perspective for those that master software development and pacakge submission and I hope to learn more with the onset of future rotations.
